...
vb_bs_corr <- function(y, B, m = 5, mu_ki = 1/2, lambda_1 = 10^(-10), lambda_2 = 10^(-10), delta_1 = 10^(-10), delta_2 = 10^(-10), maxIter = 1000, K = 10, initial_values, convergence_threshold = 0.01, Xt = seq(0, 1, length = times)){
  ...
  while(converged == FALSE & iter_t < maxIter){
    ...
    while(converged_ELBO == FALSE & iter < maxIter){
      ...
      delta2_q <- (sum(sapply(1:m, function(i){E_square_gi(B = B, i = i, y = y, mu = mu_beta_values, Sigma = Sigma_beta, p = p_values, iter = iter, psi = psi)})) + sum(sapply(1:m, function(i){E_sumk_beta_i_corr(i = i, mu = mu_beta_values, Sigma = Sigma_beta,  iter = iter)}))*E_inv_tau2(lambda1 = lambda1_q,lambda2 = lambda2_values[iter - 1]) + 2*delta2_values[1])/2
      
      delta2_values[iter] <- delta2_q
      
      # tau2 posterior parameter
      lambda2_q <- (sum(sapply(1:m, function(i){E_sumk_beta_i_corr(i = i, mu = mu_beta_values, Sigma = Sigma_beta,  iter = iter)}))*E_inv_sigma2(delta1 = delta1_q, delta2 = delta2_q) + 2*lambda2_values[1])/2
      ...
      for(i in 1:m){
        ...
        for(k in 1:K){
          ...
          if(sum(exp(log_rho_ki)) == 0){
            ...
            p_ki_1 <- exp(((-ni[i]/2)*E_log_sigma2(delta1 = delta1_q, delta2 = delta2_q)-0.5*E_inv_sigma2(delta1 = delta1_q, delta2 = delta2_q)*E_square_beta_i(z = 1, i = i, p = p_star, mu = mu_beta_values, Sigma = Sigma_beta, B = B, y = y, k = k, K = K, iter = iter, psi = psi) + E_log_theta_ki(a1_ki = a1_ki_q, a2_ki = a2_ki_q)))/sum(exp(log_rho_ki))
          }
      ...
      converged_ELBO <- check_convergence(elbo_c, elbo_prev, convergence_threshold)
    ...
    out <- list(mu_beta = mu_beta_values[iter,], Sigma_beta = Sigma_beta, p = p_values[iter, ], lambda1 = lambda1_q, lambda2 = lambda2_values[iter], delta1 = delta1_q, delta2 = delta2_values[iter], a1 = a1_values[iter,], a2 = a2_values[iter,])
    ...
    # w_c <- mean(sapply(1:m, function(i){
    #   p_i <- out$p[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
    #   mu_bi <- out$mu_beta[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
    #   var_bi <- diag(out$Sigma_beta[,,i])
    #   n_i <- ni[[i]]
    #   var_beta_z <- t(var_bi + mu_bi^2)%*%(p_i*(1-p_i)*t(B[[i]])^2) + t(var_bi)%*%(p_i*(1-p_i)*t(B[[i]])^2) + t(mu_bi^2)%*%(p_i*(1-p_i)*t(B[[i]])^2)
    #   Emui <- t(p_i*t(B[[i]]))%*%mu_bi
    #   
    #   FUN <- function(w){
    #     psi <- calPsi(Xt, Xt, w = w)
    #     -sum(sapply(1:m, (-0.5*n_i*(log(2*pi) + E_log_sigma2(out$delta1, out$delta2)) - 0.5*log(det(psi)) - 0.5*(out$delta1/out$delta2)*(t(y[[i]]-Emui)%*%solve(psi)%*%(y[[i]]-Emui) + sum(diag(solve(psi)%*%diag(var_beta_z[1,])))))))
    #   }
    
    
    FUN <- function(w, Xt, out){
      psi <- calPsi(Xt, Xt, w = w)

      outt <- sum(sapply(1:m, function(i){
        p_i <- out$p[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
        mu_bi <- out$mu_beta[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
        Sigmai <- out$Sigma_beta[,,i]
        var_bi <- diag(Sigmai)

        ai_1 <- out$a1[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))] 
        ai_2 <- out$a2[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
        ...
        #Var_zi_betai_true <- diag(z_i*(1-z_i)) + diag(z_i*(1-z_i))*Sigmai + diag(z_i*(1-z_i))*(coef%*%t(coef)) + Sigmai*(z_i%*%t(z_i))

        #t(mu_bi)%*%(p_i*(1-p_i)*t(B[[i]]^2))%*%mu_bi + t(p_i*t(B[[i]]))%*%Sigmai%*%(p_i*t(B[[i]])) + sum(diag(t(Sigmai)%*%(p_i*(1-p_i)*t(B[[i]]^2))))


        #delta2 <- (sum(sapply(1:m, function(i){E_square_gi(B = B, i = i, y = y, mu = out$mu_beta, Sigma = out$Sigma_beta, p = out$p, iter = 1, psi = psi)})) + sum(sapply(1:m, function(i){E_sumk_beta_i_corr(i = i, mu = out$mu_beta, Sigma = out$Sigma_beta,  iter = 1)}))*E_inv_tau2(lambda1 = out$lambda1, lambda2 = out$lambda2) + 2*delta2_values[1])/2
        ...
        (-0.5*n_i*(log(2*pi) + E_log_sigma2(out$delta1, out$delta2)) - 0.5*log(det(psi)) - 0.5*E_inv_sigma2(out$delta1, out$delta2)*(t(y[[i]] - t(p_i*t(B[[i]]))%*%mu_bi)%*%solve(psi)%*%(y[[i]] - t(p_i*t(B[[i]]))%*%mu_bi) + sum(diag(solve(psi)%*%(B[[i]]%*%Var_zi_betai%*%t(B[[i]]))))))})) 
      ...
      #     ai_2 <- out$a2[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
      # 
      #     sum(sapply(1:K, function(k){p_i[k]*E_log_theta_ki(ai_1[k], ai_2[k]) + (1-p_i[k])*E_log_theta_ki_c(ai_1[k], ai_2[k])}))})) +
      #    sum(sapply(1:m, function(i){sum(sapply(1:K, function(k){
      #      p_i <- out$p[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
      #      ai_1 <- out$a1[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))] 
      #      ai_2 <- out$a2[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
      # 
      #      (mu_ki - 1)*E_log_theta_ki(ai_1[k], ai_2[k]) + (1-mu_ki-1)*E_log_theta_ki_c(ai_1[k], ai_2[k]) - 
      #       log(beta(mu_ki, 1-mu_ki))}))})) + 
      #    (-m*K/2)*(log(2*pi) + E_log_sigma2(out$delta1, out$delta2) + E_log_tau2(out$lambda1, out$lambda2)) + sum(sapply(1:m, function(i){
      #      mu_bi <- out$mu_beta[grep(paste0("^beta_[0-9]+_",i,"$"), names(out$mu_beta))]
      #      Sigmai <- out$Sigma_beta[,,i]
      #      var_bi <- diag(Sigmai)
      #      
      #      -0.5*E_inv_sigma2(out$delta1, out$delta2)*E_inv_tau2(out$lambda1, out$lambda2)*sum(mu_bi^2 + var_bi^2)})) + (delta_1*log(delta_2) - lgamma(delta_1) - (delta_1 + 1)*E_log_sigma2(out$delta1, out$delta2) - delta_2*E_inv_sigma2(out$delta1, out$delta2)) + (lambda_1*log(lambda_2) - lgamma(lambda_2) - (lambda_1 + 1)*E_log_tau2(out$lambda1, out$lambda2) - lambda_2*E_inv_tau2(out$lambda1, out$lambda2))

      #(-0.5*n_i*(log(2*pi) + log(sigma^2)) -0.5*log(det(psi)) - 0.5*(1/sigma^2)*(t(y[[i]] - t(z_i*t(B[[i]]))%*%coef)%*%solve(psi)%*%(y[[i]] - t(z_i*t(B[[i]]))%*%coef) + sum(diag(solve(psi)%*%(B[[i]]%*%Var_zi_betai_true%*%t(B[[i]]))))))}))



        #(t(y[[i]]-Emui)%*%solve(psi)%*%(y[[i]]-Emui) + sum(diag(solve(psi)%*%(B[[i]]%*%Var_zi_betai%*%t(B[[i]]))))))}))
    ...
    #w_c <- optim(par = initial_values$w, fn = FUN,  method = "L-BFGS-B", control = list(fnscale = -1), lower = 0, upper = .Machine$double.eps**-1)$par
    
    #w_c <- optim(par=initial_values$w, fn = FUN, method="L-BFGS-B", lower = 0, upper = Inf, Xt = Xt, out=out)$par
    w_c <- optimize(FUN, Xt=Xt, out=out, lower = 0, upper = 100)$minimum
    #}))
    ...
    converged_EM <- ifelse(abs(w_prev - w) <= 0.001, TRUE, FALSE)
    converged <- ifelse(converged_ELBO == FALSE | converged_EM == FALSE, FALSE, TRUE)
  ...
  #converged_ElBO <- check_convergence(elbo_c, elbo_prev, convergence_threshold)
  #converged_EM <- ifelse(abs(w_prev - w) <= 0.1, TRUE, FALSE)
  ...
  if(converged == TRUE){
    ...
    return(list(mu_beta = mu_beta_values[iter,], Sigma_beta = Sigma_beta, p = p_values[iter, ], lambda1 = lambda1_q,
                lambda2 = lambda2_values[iter], delta1 = delta1_q, delta2 = delta2_values[iter], a1 = a1_values[iter,], a2 = a2_values[iter,],
                conv_elbo = elbo_c, n_iterations = iter_t, elbo_values = ELBO_values, w = w))
  } else{
