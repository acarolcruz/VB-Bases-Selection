...
vb_bs_corr_fixw <- function(y, B, m = 5, mu_ki = 1/2, lambda_1 = 10^(-10), lambda_2 = 10^(-10), delta_1 = 10^(-10), delta_2 = 10^(-10), maxIter = 1000, K = 10, initial_values, convergence_threshold = 0.01, Xt = seq(0, 1, length = times), w = w){
  ...
  while(converged_ELBO == FALSE & iter < maxIter){
    ...
    for(i in 1:m){
      ...
      for(k in 1:K){
        ...
        #p_ki_1 <- exp(log_rho_ki)/(1+exp(log_rho_ki))
