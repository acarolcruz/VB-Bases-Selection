...
vb_bs <- function(y, B, m = 5, mu_ki = 1/2, lambda_1 = 10^(-10), lambda_2 = 10^(-10), delta_1 = 10^(-10), delta_2 = 10^(-10), maxIter = 1000, K = 10, initial_values, convergence_threshold = 0.01){
  ...
  while(converged == FALSE & iter < maxIter){
    ...
    delta2_q <- (sum(sapply(1:m, function(i){E_square_i(B = B, i = i, y = y, mu = mu_beta_values, Sigma = Sigma_beta, p = p_values, iter = iter)})) + sum(sapply(1:m, function(i){E_sumk_beta_i(i = i, mu = mu_beta_values, Sigma = Sigma_beta,  iter = iter)}))*E_inv_tau2(lambda1 = lambda1_q,lambda2 = lambda2_values[iter - 1]) + 2*delta_2)/2
    
    delta2_values[iter] <- delta2_q
    
    lambda2_q <- (sum(sapply(1:m, function(i){E_sumk_beta_i(i = i, mu = mu_beta_values, Sigma = Sigma_beta,  iter = iter)}))*E_inv_sigma2(delta1 = delta1_q, delta2 = delta2_q) + 2*lambda_2)/2
  ...
  if(iter == maxIter & converged == FALSE){cat("The algorithm did not converge and the max iteration has been achieved")}
   ...
   if(converged == TRUE){
     return(list(data = y, B = B, mu_beta = mu_beta_values[iter,], Sigma_beta = Sigma_beta[[iter]], p = p_values[iter, ], lambda1 = lambda1_q,
